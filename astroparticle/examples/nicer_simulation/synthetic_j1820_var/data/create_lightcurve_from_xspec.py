...
def generate_sample_varmodel(num_timesteps,
                             ndim,
                             arorder=1,
                             noise_scale=0.3,
                             random_seed=123):

    coefs = np.tile(np.diag(np.repeat(0.1, ndim)), (arorder, 1, 1))
...
class MyPhysicalModel:
    ...
    def __init__(self, energy_edges, x):
        ...
        self.diskbb = aps.DiskBB(energy_edges, x[2], x[3])
        self.phabs = aps.PhabsNicerXti(energy_edges, x[4])
...
def main():
    ...
    params = generate_sample_varmodel(
        num_timesteps=num_timesteps, ndim=4)
    params = np.exp(params)
    params = tf.convert_to_tensor(
        params * np.array([1.0, 10., 0.2, 1e6]),
        dtype=tf.float32)
    params = tf.concat([params,
                        tf.repeat(0.5, num_timesteps)[:, tf.newaxis]],
                       axis=-1)
